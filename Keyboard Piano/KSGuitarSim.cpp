...
#define CONCERT_A 220.0  // 220 for piano and guitar //520 for drum
#define SAMPLES_PER_SEC 44100  // 4100 for drum
...
int main() {
    ...
    for (int i = 0; i < (signed)keyboard.size(); i++) {
        // 24 for guitar //8 for piano //24 for guitar
        frequency = CONCERT_A * pow(2, ((i - 24) / 12.0));
        StringSound tmp = StringSound(frequency);
